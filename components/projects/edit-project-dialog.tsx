...
import { z } from "zod"
import { User, MapPin, CalendarIcon, DollarSign, Loader2, Edit } from "lucide-react"
...
export function EditProjectDialog({
  ...
  const form = useForm<ProjectFormData>({
    ...
    defaultValues: {
      ...
      payroll_budget: project.payroll_budget || undefined,
  ...
  const onSubmit = async (data: ProjectFormData) => {
    ...
    try {
      const result = await updateProject(userId, project.id, {
        ...data,
      ...
      <DialogContent className="sm:max-w-[600px]">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <FormSection title="Budget" columns={2}>
              <FormField
                control={form.control}
                name="budget"
                render={({ field }) => (
